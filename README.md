...
**Текущий статус:** Фаза 3, Этапы 3.1-3.3 завершены. Предстоящие этапы:
- ✅ Этап 3.1: Файловые операции (загрузка/сохранение .mtl файлов) — завершён
- ✅ Этап 3.2: Поддержка строковых ачисел (.astr) — завершён
- ✅ Этап 3.3: Поддержка четверичной нотации (.anum) — завершён
...
## Четверичная нотация (.anum) — v0.4.0

Поддержка четверичной нотации позволяет работать с базовой сериализацией МТС:

### Что такое четверичное ачисло?

Четверичное ачисло — это последовательность из четырёх абитов, интерпретируемая как цепочка связей:

```
"[1]" ≡ (((∞ -> ♂∞) -> (♂∞ -> ∞♀)) -> ∞♀)
```

Четыре абита представляют базовые формы вокруг акорня ∞:

| Абит | Форма | Описание |
|------|-------|----------|
| `[`  | `♂∞`  | Самозамыкание начала (Male) |
| `]`  | `∞♀`  | Самозамыкание конца (Female) |
| `1`  | `♂∞ -> ∞♀` | Направленная связь |
| `0`  | `∞♀ -> ♂∞` | Инвертированная связь |
...
При загрузке `.anum` файла можно включить панель визуализации (кнопка "Show Conv"), которая показывает пошаговое преобразование абитов в цепочку формальных запросов с отображением формы каждого абита.
...
│   │   └── quatAnum.ts      # Парсер четверичной нотации (.anum)
│   ├── components/
...
│   └── basic.anum           # Примеры четверичной нотации
├── index.html
├── package.json
├── tsconfig.json
├── vite.config.ts
